import { startCar, drive, stopCar } from './api/api';
...
export class Car extends BaseComponent {
    ...
    constructor(
        ...
        private driveCar: () => number,
        private showName: (name: string) => void,
        private createWinner: (id: number, name: string, color: string, progress: number) => void
    ) {
        super({
            classNames: ['cars__item'],
        });
        this.model = name;
        this.colorCar = color;
        const carWrap = new BaseComponent({
            classNames: ['car-wrapper'],
        });
        this.insertChild(carWrap);
        const driveBtn = new BaseComponent({
            classNames: ['drive-btn'],
        });
        carWrap.insertChild(driveBtn);
        const start = new BaseComponent({
            tagName: 'button',
            classNames: ['start'],
            textContent: 'S',
        });
        driveBtn.insertChild(start);
        const end = new BaseComponent({
            tagName: 'button',
            classNames: ['end'],
            textContent: 'E',
        });
        end.setAttribute('disabled', 'true');
        driveBtn.insertChild(end);
        const carInformation = new BaseComponent({
            classNames: ['car'],
        });
        carWrap.insertChild(carInformation);
        const control = new BaseComponent({
            classNames: ['control'],
        });
        carInformation.insertChild(control);
        const select = new BaseComponent({
            tagName: 'button',
            classNames: ['control__select'],
            textContent: 'select',
        });
        control.insertChild(select);
        const remove = new BaseComponent({
            tagName: 'button',
            classNames: ['control__remove'],
            textContent: 'remove',
        });
        control.insertChild(remove);
        this.img = document.createElement('div');
        this.img.classList.add('car__img');
        const nameCar = new BaseComponent({
            classNames: ['car__name'],
            textContent: name,
        });
        carInformation.insertChild(nameCar);
        this.img.innerHTML = `<svg version="1.0" xmlns="http://www.w3.org/2000/svg"
       ...
       130 22 193 51 l64 29 -19 23 c-65 82 -198 227 -209 227 -7 0 -15 -4 -19 -9z"/>
        ...
        carInformation.insertChildS(this.img);
        const finish = new BaseComponent({
            tagName: 'img',
            classNames: ['car__finish'],
        });
        finish.setAttribute('src', 'https://cdn-icons-png.flaticon.com/512/2052/2052749.png');
        carWrap.insertChild(finish);
        const line = new BaseComponent({
            classNames: ['car__race'],
        });
        this.insertChild(line);
        ...
        start.addListener('click', () => {
            ...
            const id = driveCar();
            startCar(id).then((v) => {
                this.requestId = window.requestAnimationFrame(this.amimationCar.bind(this, v.velocity));
                drive(id).then((g) => {
                    if (g.success === false) this.go = false;
                });
            });
        });
            ...
            this.go = false;
            stopCar(id).then(() => {
                this.img.style.transform = `translateX(0px)`;
            });
            this.animationStart = 0;
        });
    }
    amimationCar(velocity: number, timestamp: number): void {
        if (!this.animationStart) {
            this.animationStart = timestamp;
        }
        const progress = timestamp - this.animationStart;
        this.img.style.transform = `translateX(${(progress / 1000) * velocity}px)`;
        const x = this.img.getBoundingClientRect().x + 100;
        if (x >= window.innerWidth - 100 && localStorage['flag'] == 'true') {
            this.showName(this.model);
            this.createWinner(this.driveCar(), this.model, this.colorCar, progress);
            localStorage['flag'] = 'false';
        }
        if (x <= window.innerWidth - 100 && this.go !== false) {
            window.requestAnimationFrame(this.amimationCar.bind(this, velocity));
        } else {
            window.cancelAnimationFrame(<number>this.requestId);
        }
    }
}
