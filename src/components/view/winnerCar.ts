...
import './winnerCar.scss';

export class WinnerCar extends BaseComponent {
    constructor(id: number, name: string, color: string, wins: number, time: number) {
        super({
            tagName: 'tr',
            classNames: ['winners__item'],
        });
        const number = new BaseComponent({
            tagName: 'td',
            classNames: ['winners__item_text'],
            textContent: `${id}`,
        });
        this.insertChild(number);
        const img = document.createElement('th');
        img.classList.add('car__img');
        img.innerHTML = `<svg version="1.0" xmlns="http://www.w3.org/2000/svg"
       ...
       130 22 193 51 l64 29 -19 23 c-65 82 -198 227 -209 227 -7 0 -15 -4 -19 -9z"/>
        ...
        this.insertChildS(img);
        const nameCar = new BaseComponent({
            tagName: 'td',
            classNames: ['winners__item_text'],
            textContent: `${name}`,
        });
        this.insertChild(nameCar);
        const winsCar = new BaseComponent({
            tagName: 'td',
            classNames: ['winners__item_text'],
            textContent: `${wins}`,
        });
        this.insertChild(winsCar);
        const timeCar = new BaseComponent({
            tagName: 'td',
            classNames: ['winners__item_text'],
            textContent: `${time}`,
        });
        this.insertChild(timeCar);
    }
}
